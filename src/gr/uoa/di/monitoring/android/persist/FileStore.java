...
public final class FileStore {
	...
	/**
	 * Persists the given data in {@code file} in the given {@code file} which
	 * may lie in external or internal storage. Each item (chunk of bytes) is
	 * separated by the next one by DELIMITER. The items may be either a byte[],
	 * an element of the {@code listByteArrays}, or a whole List<byte[]> (member
	 * of {@code listsOfByteArrays}). In the latter case each byte[] belonging
	 * to {@code listsOfByteArrays} is persisted in turn and separated by the
	 * next by ARRAY_DELIMITER. The last array is separated by the next chunk by
	 * DELIMITER as before. The order of the items is given by the order of the
	 * Lists themselves in combination with the in {@code fields} which also
